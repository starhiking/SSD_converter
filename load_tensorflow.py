...
with tf.Session(graph=tf.Graph()) as sess:
    tf.saved_model.loader.load(sess, [tf.saved_model.tag_constants.TRAINING], './ssd.pb')
    ...
    conv17_2_mbox_conf = sess.graph.get_tensor_by_name('add_11:0')
    conv17_2_mbox_loc = sess.graph.get_tensor_by_name('add_10:0')

    result = sess.run([conv17_2_mbox_loc,conv17_2_mbox_conf],feed_dict={input_x:transform_img})

    print(result[0].reshape(-1))
    print(result[1].reshape(-1))
